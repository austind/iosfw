...
class iosfw(object):
    ...
    def _get_src_path(self, file_name=None, local=False):
        ...
        if proto == "scp" or local:
            ...
            return "{}://{}:{}@{}{}{}".format(
                proto, un, pw, src, path, file_name
            )
        else:
    ...
    def get_boot_image(self):
        ...
        if "Invalid input" in output:
            self.log.debug(
                "Device does not support `show boot`. "
    ...
    def request_install(self):
        ...
        if self.upgrade_method == "manual":
            ...
            if "Error" in output:
                ...
                "All software images installed" in output
            ...
            if self.old_images and self.can_delete_old_images:
                ...
                if self.can_delete_running_image:
                    ...
                    if self.ft.verify_space_available():
                        ...
                        self.log.critical(
                            "Still not enough space." "Cannot continue."
                        )
                        return False
                else:
                    self.log.critical(
    ...
    def upgrade(self):
        ...
        if self.needs_upgrade and not self.firmware_installed:
            ...
            self.log.info(
                "Upgrade on {} {} at {}".format(self.hostname, status, end)
            )
